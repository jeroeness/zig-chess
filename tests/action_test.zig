...
test "action and move functionality" {
    ...
    const piece_at_start = chess_board.getPieceConst(Coord.init(1, 4));
    ...
    try testing.expect(chess_board.isEmpty(Coord.init(3, 4)));
    ...
    const piece_at_target = chess_board.getPieceConst(Coord.init(3, 4));
    ...
    const piece_back_at_start = chess_board.getPieceConst(Coord.init(1, 4));
...
test "action demo - comprehensive move execution and undo" {
    ...
    try testing.expect(game_board.isEmpty(Coord.init(1, 4))); // e2 should be empty
    try testing.expect(!game_board.isEmpty(Coord.init(3, 4))); // e4 should have piece
    const piece_at_e4 = game_board.getPieceConst(Coord.init(3, 4));
    ...
    // Verify the move was executed correctly
    try testing.expect(game_board.isEmpty(Coord.init(6, 4))); // e7 should be empty
    try testing.expect(!game_board.isEmpty(Coord.init(4, 4))); // e5 should have piece
    const piece_at_e5 = game_board.getPieceConst(Coord.init(4, 4));
    ...
    try testing.expect(!game_board.isEmpty(Coord.init(6, 4))); // e7 should have piece again
    try testing.expect(game_board.isEmpty(Coord.init(4, 4))); // e5 should be empty
    const piece_back_at_e7 = game_board.getPieceConst(Coord.init(6, 4));
    ...
    try testing.expect(!game_board.isEmpty(Coord.init(1, 4))); // e2 should have piece again
    try testing.expect(game_board.isEmpty(Coord.init(3, 4))); // e4 should be empty
    const piece_back_at_e2 = game_board.getPieceConst(Coord.init(1, 4));
...
test "pawn promotion - all piece types including amazon" {
    ...
    // Test promotion to queen
    {
        ...
        _ = game_board.setPiece(Coord.init(6, 4), Piece.init(.pawn, .white, 1));
        ...
        const promoted_piece = game_board.getPieceConst(Coord.init(7, 4));
        ...
        try testing.expect(game_board.isEmpty(Coord.init(6, 4)));
        ...
        try testing.expect(game_board.isEmpty(Coord.init(7, 4))); // Target should be empty
        const restored_piece = game_board.getPieceConst(Coord.init(6, 4));
    ...
    // Test promotion to amazon (fairy piece)
    {
        ...
        _ = game_board.setPiece(Coord.init(1, 3), Piece.init(.pawn, .black, 1));
        ...
        // Verify the promoted piece
        const promoted_piece = game_board.getPieceConst(Coord.init(0, 3));
        try testing.expect(promoted_piece != null);
        try testing.expect(promoted_piece.?.getType() == .amazon);
        try testing.expect(promoted_piece.?.getColor() == .black);

        // Verify the original position is empty
        try testing.expect(game_board.isEmpty(Coord.init(1, 3)));
        ...
        // Verify undo worked correctly
        try testing.expect(game_board.isEmpty(Coord.init(0, 3))); // Target should be empty
        const restored_piece = game_board.getPieceConst(Coord.init(1, 3));
        try testing.expect(restored_piece != null);
        try testing.expect(restored_piece.?.getType() == .pawn);
    ...
    // Test promotion with capture
    {
        ...
        _ = game_board.setPiece(Coord.init(7, 4), Piece.init(.rook, .black, 2));
        ...
        const restored_captured = game_board.getPieceConst(Coord.init(7, 4));
...
test "skip action functionality" {
    ...
    const initial_piece_e2 = game_board.getPieceConst(Coord.init(1, 4));
    const initial_piece_e7 = game_board.getPieceConst(Coord.init(6, 4));
    ...
    // Test white skip
    {
        ...
        const piece_e2_after = game_board.getPieceConst(Coord.init(1, 4));
        const piece_e7_after = game_board.getPieceConst(Coord.init(6, 4));
        ...
        const piece_e2_after_undo = game_board.getPieceConst(Coord.init(1, 4));
        const piece_e7_after_undo = game_board.getPieceConst(Coord.init(6, 4));
