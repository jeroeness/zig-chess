...
pub const Board = struct {
    ...
    pub fn getCell(self: *Board, row: u8, col: u8) ?*Cell {
        if (row >= 8 or col >= 8) {
            return null;
        }
    ...
    pub fn isEmpty(self: *const Board, row: u8, col: u8) bool {
        if (row >= 8 or col >= 8) {
            return true;
        }
